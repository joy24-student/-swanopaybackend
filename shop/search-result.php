    ...
    <div class="inner">
        <h1>
            ...
            <?php 
                ...
                echo htmlspecialchars($search_text, ENT_QUOTES, 'UTF-8'); 
            ?>            
        </h1>
    </div>
</div>
...
<div class="page">
    <div class="container">
        <div class="row">
            ...
            $statement->execute(array(1,$search_text_like));
            $total_pages = $statement->rowCount();

            $targetpage = BASE_URL.'search-result.php?search_text='.urlencode($search_text);   //your file name  (the name of this file)
            $limit = 12;                                 //how many items to show per page
            $page = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
            $start = ($page - 1) * $limit;

            $statement = $pdo->prepare("SELECT * FROM tbl_product WHERE p_is_active=? AND p_name LIKE ? LIMIT ? OFFSET ?");
            $statement->bindValue(1, 1, PDO::PARAM_INT);
            $statement->bindValue(2, $search_text_like, PDO::PARAM_STR);
            $statement->bindValue(3, (int)$limit, PDO::PARAM_INT);
            $statement->bindValue(4, (int)$start, PDO::PARAM_INT);
            $statement->execute();
            $result = $statement->fetchAll(PDO::FETCH_ASSOC);

...
if [ -f "$TARGET_DIR/swapnopay-backend/package.json" ]; then
    ...
    if [ -d "$TARGET_DIR/swapnopay-backend" ] && [ ! -f "$TARGET_DIR/swapnopay-backend/package.json" ]; then
        rm -rf "$TARGET_DIR/swapnopay-backend"
    fi
fi

# 3. Ensure required directories and permissions
echo -e "${YELLOW}📁 Verifying storage directories...${NC}"
STORAGE_BASE="${BACKEND_APP_DIR:-$TARGET_DIR}"
mkdir -p "$STORAGE_BASE/uploads/kyc"
mkdir -p "$STORAGE_BASE/uploads/products"
mkdir -p "$STORAGE_BASE/data/shop-runtime/hosts"
mkdir -p "$STORAGE_BASE/data/shop-sites/hosts"
mkdir -p "$STORAGE_BASE/data/shop-sites/stores"
chown -R www-data:www-data "$STORAGE_BASE/data/shop-runtime" "$STORAGE_BASE/data/shop-sites" 2>/dev/null || true
chmod -R 775 "$STORAGE_BASE/uploads" "$STORAGE_BASE/data" 2>/dev/null || true

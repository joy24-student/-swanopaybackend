...
serve(async (request) => {
  ...
  try {
    ...
    const responseCustomVariables = buildCustomVariableAssignments(fields, validation.answers, result);
    ...
    return json({
      ...
      custom_variables: responseCustomVariables,
    }, 201);
  } catch (error) {
...
function resolveTemplateExpression(expression: string, ctx: Record<string, string>): string {
  return String(expression || "").replace(/{{\s*([a-zA-Z0-9_]+)\s*}}/g, (_, key) => String(ctx[key] ?? ""));
}

function buildCustomVariableAssignments(fields: FormField[], answers: Json, result: Json): Record<string, string> {
  const ctx: Record<string, string> = {};
  for (const [key, value] of Object.entries(asObject(answers))) {
    ctx[String(key)] = String(value ?? "");
  }
  for (const [key, value] of Object.entries(asObject(result))) {
    ctx[String(key)] = String(value ?? "");
  }
  const assignments: Record<string, string> = {};
  for (const field of fields) {
    const entryMap = asObject((field as Json).custom_variable_assignments || (field as Json).customVariableAssignments || {});
    for (const [name, expression] of Object.entries(entryMap)) {
      const normalized = String(name || "").replace(/[^a-zA-Z0-9_]/g, "_").replace(/^_|_$/g, "");
      if (!normalized) continue;
      assignments[normalized] = resolveTemplateExpression(String(expression || ""), ctx);
    }
  }
  return assignments;
}
...
function renderForm(form: Json, methodRows: Json[], nonce: string): string {
  ...
  function contextFrom(payload,response){const ctx={...customDefaults,name:'',email:'',phone:'',submission_id:response.submission_id||'',transaction_id:response.transaction_id||'',payment_status:response.payment_status||'NOT_REQUIRED',payment_amount:response.amount||0,payment_method:response.payment_method||'',created_at:new Date().toISOString(),all_fields:JSON.stringify(payload.answers)};const customVars = response.custom_variables || {};Object.entries(customVars).forEach(([key,value])=>{ctx[key]=String(value ?? '')});document.querySelectorAll('[data-field-id]').forEach(el=>{const key='field_'+String(el.dataset.fieldLabel||el.dataset.fieldId).toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'');const value=payload.answers[el.dataset.fieldId]||'';ctx[key]=value;if(el.dataset.fieldType==='NAME')ctx.name=value;if(el.dataset.fieldType==='EMAIL')ctx.email=value;if(el.dataset.fieldType==='PHONE')ctx.phone=value});return ctx}
...
function renderField(field: FormField, enforceRequired: boolean, requiredIndicator: boolean, theme: Json): string {
  ...
  if (type === "CUSTOM_CODE") { const css = sanitizeCss(String(field.custom_code_css || "")), html = sanitizeCustomHtml(String(field.custom_code_html || "")); const assignmentJson = JSON.stringify(asObject(field.custom_variable_assignments || field.customVariableAssignments || {})); return `${css ? `<style>${css}</style>` : ""}<section class="code-block" data-code-template="${escapeHtml(html)}" data-custom-assignments="${escapeHtml(assignmentJson)}"></section>`; }
